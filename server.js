...
const dbDictionary = new sqlite3.Database(
  dbPath,
...
app.get("/search/:word", (req, res) => {
  const sql =
    "SELECT lemma, synonyms, antonyms, definition FROM words WHERE lemma = ?";
  ...
  dbDictionary.get(sql, [req.params.word], (err, row) => {
    ...
    if (row) {
      ...
      dbDictionary.run("INSERT INTO search_history (term) VALUES (?)", [
        req.params.word,
      ]);
      res.json(row);
    } else {
